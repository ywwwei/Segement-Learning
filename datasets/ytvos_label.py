...
class YTVOSDataset:
    def __init__(self, img_folder, ann_file, transforms, return_masks, num_frames, training_percent=1):
        ...
        n_img = int(len(self.img_ids)*training_percent)
        self.img_ids = self.img_ids[:n_img]
    def __len__(self):
...
def build(image_set, args):
    ...
    if args.dataset_file == 'YoutubeVIS' or args.dataset_file == 'jointcoco':
        ...
        training_percent = args.training_percent if image_set == 'train' else 1
        dataset = YTVOSDataset(img_folder, ann_file, transforms=make_coco_transforms(image_set), return_masks=args.masks,  num_frames = 1, training_percent = training_percent)
